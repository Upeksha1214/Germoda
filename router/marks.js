  ...
  try {
    ...
    if (re == null) {
      ...
      response != null
        ? res.json({
            code: "200",
            message: "Marks add is  successfull",
            data: null,
          })
      ...
      res.json({
        code: "500",
        message: "Class name or student is AllreadyExists",
        data: null,
      });
    }
  } catch (error) {
...
router.put("/marksUpdate/:examId", async (req, res) => {
  try {
    const response = await Marks.findOneAndUpdate(
...
router.delete("/marksDelete/:examId", async (req, res) => {
  try {
    const response = await Marks.findOneAndDelete(
      { examId: req.params.examId },
      req.body
    );
    response != null
      ? res.json({
          code: "200",
          message: "Marks delete successfull",
          data: null,
        })
...
router.get("/marksAll", async (req, res) => {
  try {
    ...
    re != null
      ? res.json({ code: "200", message: "Marks getAll successfull", data: re })
      : res.json({
          code: "500",
          message: "Marks getall delete Fail",
          data: null,
        });
  } catch (error) {
    res.send("Err" + error);
